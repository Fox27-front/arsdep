        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 'pwdMissMatch') {
                    $(".pwd-retype").text("Введите тот же пароль");
                    $('#re_pass').val('')
                } else if (data == 'sqlError') {
                    $(".errorLogs").addClass("error");
                    $(".errorLogs").text("Ошибка SQL");
                    $('#register-form').find("input").val('')
                } else if (data == 'usernameTaken') {
                    $(".uName").text("Это имя пользователя уже существует, попробуйте другое");
                    $('#username').val('')
                } else if (data == 'Error') {
                    $(".errorLogs").addClass("error");
                    $(".errorLogs").text("При регистрации произошла ошибка");
                } else if (data == 'OK') {
                    $(".errorLogs").removeClass("error");
                    $(".errorLogs").addClass("success");
                    $('#register-form').find("input").val('')
                    $(".errorLogs").text("Вы успешно зарегистрированы");
                    $('#register').text("Предоставление ...")
                    setTimeout(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data == 'wrongPwd') {
                    ...
                    $(".pwd").text("Неверный пароль");
                    $('#password').val('')
                } else if (data == 'noUser') {
                    $(".loginErrorLogs").addClass("error");
                    $(".loginErrorLogs").text("Пользователь не найден!. Пожалуйста зарегистрируйтесь");
                    $('#login-form').find("input").val('')
                } else if (data == 'OK') {
                    $(".loginErrorLogs").removeClass("error");
                    $(".loginErrorLogs").addClass("success");
                    $('#login-form').find("input").val('')
                    $(".loginErrorLogs").text("Успешный вход в систему");
                    $('#login').text("Регистрация ...")
                    setTimeout(function () {
                        $(location).attr('href', 'welcome.php')
                    }, 3500);
                }
